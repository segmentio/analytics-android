...
/* package */final class DatapointHelper {
   ...
   * @return An 1-way hashed version of the {@link android.provider.Settings.Secure#ANDROID_ID}. May
   * return null if an Android
  ...
  /**
   * Gets a 1-way hashed value of the device's unique serial number. This value is encoded using a
   ...
   * <p>
   * Note: this method will return null if {@link android.Manifest.permission#READ_PHONE_STATE} is
   * not available. This method will also return
   * null on devices that do not have telephony.
   *
   * @param context The context used to access the phone state.
   * @return An the {@link android.telephony.TelephonyManager#getDeviceId()}. Null if an ID is not
   * available, or if
   * {@link android.Manifest.permission#READ_PHONE_STATE} is not available.
   */
  public static String getTelephonyDeviceIdOrNull(final Context context) {
    if (Constants.CURRENT_API_LEVEL >= 7) {

      final Boolean hasTelephony =
          ReflectionUtils.tryInvokeInstance(context.getPackageManager(), "hasSystemFeature",
              STRING_CLASS_ARRAY, HARDWARE_TELEPHONY); //$NON-NLS-1$
      if (!hasTelephony.booleanValue()) {
        if (Constants.IS_LOGGABLE) {
          Log.i(Constants.LOG_TAG,
              "Device does not have telephony; cannot read telephony id"); //$NON-NLS-1$
        }
    ...
    if (context.getPackageManager()
        .checkPermission(permission.READ_PHONE_STATE, context.getPackageName())
      ...
      if (Constants.IS_LOGGABLE) {
        Log.w(Constants.LOG_TAG,
            "Application does not have permission READ_PHONE_STATE; determining device id is not possible.  Please consider requesting READ_PHONE_STATE in the AndroidManifest"); //$NON-NLS-1$
      }
    }
  ...
  public static String getNetworkType(final Context context,
    ...
    try {
      if (context.getPackageManager()
          .checkPermission(permission.ACCESS_WIFI_STATE, context.getPackageName())
          == PackageManager.PERMISSION_GRANTED) {
        final NetworkInfo wifiInfo = ((ConnectivityManager) context.getSystemService(
            Context.CONNECTIVITY_SERVICE)).getNetworkInfo(ConnectivityManager.TYPE_WIFI);
        ...
        if (Constants.IS_LOGGABLE) {
          Log.w(Constants.LOG_TAG,
              "Application does not have one more more of the following permissions: ACCESS_WIFI_STATE. Determining Wi-Fi connectivity is unavailable"); //$NON-NLS-1$
        }
      }
  ...
  /**
   * Gets the device manufacturer's name. This is only available on SDK 4 or greater, so on SDK 3
  ...
  public static String getFBAttribution(final Context context) {
    ...
    final Uri uri =
        Uri.parse("content://com.facebook.katana.provider.AttributionIdProvider"); //$NON-NLS-1$
   ...
   *
   * @param context {@link android.content.Context}. Cannot be null.
  ...
  public static String getAppVersion(final Context context) {
    ...
    try {
      ...
      if (null == versionName) {
        if (Constants.IS_LOGGABLE) {
          Log.w(Constants.LOG_TAG,
              "versionName was null--is a versionName attribute set in the Android Manifest?"); //$NON-NLS-1$
        }
  ...
  public static String getLocalyticsRollupKeyOrNull(final Context context) {
    ...
    try {
      ...
      if (applicationInfo.metaData != null) {
        Object metaData =
            (String) applicationInfo.metaData.get(Constants.LOCALYTICS_METADATA_ROLLUP_KEY);
        if (metaData instanceof String) {
   ...
   * <p>
   * Note: This implementation contains a minor bug. The returned value may be shorter than
   * expected, because it will drop any
   * leading zeros on the front of the SHA-256 string. This bug cannot be fixed because the server
   * may have already stored the
   ...
   */
    /* package */
  static String getSha256_buggy(final String string) {
