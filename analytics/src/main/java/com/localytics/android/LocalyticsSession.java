 ...
 * <ul>
 * <li>{@link android.Manifest.permission#INTERNET}</li> - Necessary to upload data to the webservice.</li>
 * </ul>
 * Permissions recommended:
 * <ul>
 * <li>{@link android.Manifest.permission#ACCESS_WIFI_STATE}</li> - Necessary to identify the type of network connection the user has. Without this
 * permission, users connecting via Wi-Fi will be reported as having a connection type of 'unknown.'</li>
 * </ul>
 * <strong>Basic Usage</strong>
 * <ol>
 * <li>In {@code Activity#onCreate(Bundle)}, instantiate a {@link com.localytics.android.LocalyticsSession} object and assign it to a global variable in
 ...
 * <li>As the user interacts with your Activity, call {@link #tagEvent(String)}, {@link #tagEvent(String, java.util.Map)} or
 ...
 * <strong>Notes</strong>
 * <ul>
 * <li>Do not call any {@link com.localytics.android.LocalyticsSession} methods inside a loop. Instead, calls such as {@link #tagEvent(String)} should
 ...
 * {@link android.content.Context#getDatabasePath(String)} directory. For security, this file directory will be created
 * {@link android.content.Context#MODE_PRIVATE}. The host application must not modify this database file. If the host application implements a
...
public class LocalyticsSession
{
     ...
     * @return HandlerThread whose {@link android.os.HandlerThread#start()} method has already been called.
     */
    ...
    /**
     * Maps an API key to a singleton instance of the {@link com.localytics.android.LocalyticsSession.SessionHandler}. Lazily initialized during construction of the
    ...
    /**
     * Opens the Localytics session. The session should be opened before {@link #tagEvent(String)}, {@link #tagEvent(String, java.util.Map)}
     * , {@link #tagEvent(String, java.util.Map, java.util.List)}, or {@link #tagScreen(String)} are called.
     * <p>
     ...
     * <p>
     * If for any reason open is called more than once without an intervening call to {@link #close()} or {@link #close(java.util.List)},
    ...
    /**
     * Closes the Localytics session. Once a session has been opened via {@link #open()} or {@link #open(java.util.List)}, close the session
    ...
    /* package */static final class SessionHandler extends Handler
    {
         ...
         * <p>
         * {@link android.os.Message#obj} is either null or a {@code Map<String, String>} containing attributes for the open.
         */
         ...
         * <p>
         * {@link android.os.Message#obj} is either null or a {@code Map<String, String>} containing attributes for the close.
         */
         ...
         * <p>
         * {@link android.os.Message#obj} is a {@link com.localytics.android.LocalyticsSession.Pair} instance. This object cannot be null.
         */
         ...
         * <p>
         * {@link android.os.Message#obj} is a {@code Runnable} to execute when upload is complete. The thread that this runnable will
         ...
         *
         * @param context The context used to access resources on behalf of the app. It is recommended to use
         *            {@link android.content.Context#getApplicationContext()} to avoid the potential memory leak incurred by maintaining
         ...
         * collected. For example, calls to {@link #tagEvent(String, java.util.Map)} and {@link #tagScreen(String)} will be recorded until
        ...
        /**
         * Open a session. While this method should only be called once without an intervening call to {@link #close(java.util.Map)},
        ...
        /**
         * Reopens a previous session. This is a helper method to {@link #open(boolean, java.util.Map)}.
         *
        ...
        /**
         * Close a session. While this method should only be called after {@link #open(boolean, java.util.Map)}, nothing bad will happen if
         * it is called and {@link #open(boolean, java.util.Map)} wasn't called. Similarly, nothing bad will happen if close is called
         * multiple times.
         * <p>
         * This method must only be called after {@link #init()} is called.
         * <p>
        ...
        /**
         * Tag a screen in a session. While this method shouldn't be called unless {@link #open(boolean, java.util.Map)} is called first,
         * this method will simply do nothing if {@link #open(boolean, java.util.Map)} hasn't been called.
         * <p>
    ...
    /**
     * Helper object to the {@link com.localytics.android.LocalyticsSession.SessionHandler} which helps process upload requests.
     */
    /* package */static final class UploadHandler extends Handler
    {
         ...
         * forwards the request back to {@link com.localytics.android.LocalyticsSession#mSessionHandler} with {@link com.localytics.android.LocalyticsSession.SessionHandler#MESSAGE_UPLOAD}.
         * <p>
         * {@link android.os.Message#obj} is a {@code Runnable} to execute when upload is complete. The thread that this runnable will
         * executed on is undefined.
         */
         ...
         * @param sessionHandler Parent {@link com.localytics.android.LocalyticsSession.SessionHandler} object to notify when uploads are completed. Cannot be null.
            ...
            try
            {
                ...
                while (cursor.moveToNext())
                {
                    try
                    {
                        ...
                        blobHeader.put(BlobHeader.KEY_ATTRIBUTES, getAttributesFromSession(provider, apiKey, getSessionIdForBlobId(provider, cursor.getLong(idColumn))));
        ...
        /* package */static JSONObject getAttributesFromSession(final LocalyticsProvider provider, final String apiKey, final long sessionId) throws JSONException
        {
            ...
            try
            {
                ...
                if (cursor.moveToFirst())
                {
                    ...
                    // Sessions table
                    result.put(BlobHeader.Attributes.KEY_CLIENT_APP_VERSION, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.APP_VERSION)));
                    result.put(BlobHeader.Attributes.KEY_DATA_CONNECTION, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.NETWORK_TYPE)));
                    result.put(BlobHeader.Attributes.KEY_DEVICE_ANDROID_ID_HASH, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_ANDROID_ID_HASH)));
                    result.put(BlobHeader.Attributes.KEY_DEVICE_COUNTRY, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_COUNTRY)));
                    result.put(BlobHeader.Attributes.KEY_DEVICE_MANUFACTURER, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_MANUFACTURER)));
                    result.put(BlobHeader.Attributes.KEY_DEVICE_MODEL, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_MODEL)));
                    result.put(BlobHeader.Attributes.KEY_DEVICE_OS_VERSION, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.ANDROID_VERSION)));
                    result.put(BlobHeader.Attributes.KEY_DEVICE_PLATFORM, BlobHeader.Attributes.VALUE_PLATFORM);
                    result.put(BlobHeader.Attributes.KEY_DEVICE_SERIAL_HASH, cursor.isNull(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_SERIAL_NUMBER_HASH)) ? JSONObject.NULL
                            : cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_SERIAL_NUMBER_HASH)));
                    result.put(BlobHeader.Attributes.KEY_DEVICE_SDK_LEVEL, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.ANDROID_SDK)));
                    result.put(BlobHeader.Attributes.KEY_LOCALYTICS_API_KEY, apiKey);
                    result.put(BlobHeader.Attributes.KEY_LOCALYTICS_CLIENT_LIBRARY_VERSION, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.LOCALYTICS_LIBRARY_VERSION)));
                    result.put(BlobHeader.Attributes.KEY_LOCALYTICS_DATA_TYPE, BlobHeader.Attributes.VALUE_DATA_TYPE);
                    result.put(BlobHeader.Attributes.KEY_CURRENT_TELEPHONY_ID, cursor.isNull(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_TELEPHONY_ID)) ? JSONObject.NULL
                            : cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_TELEPHONY_ID)));                    
                    result.put(BlobHeader.Attributes.KEY_CURRENT_ANDROID_ID, cursor.isNull(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_ANDROID_ID)) ? JSONObject.NULL
                    ...
                    result.put(BlobHeader.Attributes.KEY_LOCALE_COUNTRY, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.LOCALE_COUNTRY)));
                    result.put(BlobHeader.Attributes.KEY_LOCALE_LANGUAGE, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.LOCALE_LANGUAGE)));
                    result.put(BlobHeader.Attributes.KEY_NETWORK_CARRIER, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.NETWORK_CARRIER)));
                    result.put(BlobHeader.Attributes.KEY_NETWORK_COUNTRY, cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.NETWORK_COUNTRY)));
        ...
        /**
         * Private helper to convert an event's attributes into a {@link org.json.JSONObject} representation.
         *
         * @param provider Localytics database instance. Cannot be null.
         * @param context Application context. Cannot be null.
         * @param eventId {@link EventsDbColumns#_ID} of the event whose attributes are to be loaded.
         * @return {@link org.json.JSONObject} representing the attributes of the event. The order of attributes is undefined and may
    ...
    /**
     * Internal helper class to pass two objects to the Handler via the {@link android.os.Message#obj}.
     */
    /*
    ...
    /**
     * Internal helper class to pass three objects to the Handler via the {@link android.os.Message#obj}.
     */
