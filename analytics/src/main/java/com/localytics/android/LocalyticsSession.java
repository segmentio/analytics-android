 ...
 * <ul>
 * <li><a href="http://wiki.localytics.com/index.php?title=Developer's_Integration_Guide">Main
 * Developer's Integration Guide</a></li>
 * <li><a href="http://wiki.localytics.com/index.php?title=Android_2_Minute_Integration">Android 2
 * minute integration Guide</a></li>
 * <li><a href="http://wiki.localytics.com/index.php?title=Android_Integration_Guide">Android
 * Integration Guide</a></li>
 * </ul>
 * <p>
 * Permissions required:
 * <ul>
 * <li>{@link android.Manifest.permission#INTERNET}</li> - Necessary to upload data to the
 * webservice.</li>
 * </ul>
 * Permissions recommended:
 * <ul>
 * <li>{@link android.Manifest.permission#ACCESS_WIFI_STATE}</li> - Necessary to identify the type
 * of network connection the user has. Without this
 * permission, users connecting via Wi-Fi will be reported as having a connection type of
 * 'unknown.'</li>
 * </ul>
 * <strong>Basic Usage</strong>
 * <ol>
 * <li>In {@code Activity#onCreate(Bundle)}, instantiate a {@link com.localytics.android.LocalyticsSession}
 * object and assign it to a global variable in
 * the Activity (e.g. {@code mLocalyticsSession}).</li>
 * <li>In {@code Activity#onResume()}, call {@link #open()} or {@link #open(java.util.List)}.</li>
 * <li>In {@code Activity#onResume()}, consider calling {@link #upload()}. Because the session was
 * just opened, this upload will
 * submit that open to the server and allow you to capture real-time usage of your
 * application.</li>
 * <li>In {@code Activity#onResume()}, consider calling {@link #tagScreen(String)} to note that the
 ...
 * <li>As the user interacts with your Activity, call {@link #tagEvent(String)}, {@link
 * #tagEvent(String, java.util.Map)} or
 * {@link #tagEvent(String, java.util.Map, java.util.List)} to collect usage data.</li>
 * <li>In {@code Activity#onPause()}, call {@link #close()} or {@link #close(java.util.List)}.</li>
 * </ol>
 * <strong>Notes</strong>
 * <ul>
 * <li>Do not call any {@link com.localytics.android.LocalyticsSession} methods inside a loop.
 * Instead, calls such as {@link #tagEvent(String)} should
 * follow user actions. This limits the amount of data which is stored and uploaded.</li>
 * <li>This library will create a database called "com.android.localytics.sqlite" within the host
 * application's
 * {@link android.content.Context#getDatabasePath(String)} directory. For security, this file
 * directory will be created
 * {@link android.content.Context#MODE_PRIVATE}. The host application must not modify this database
 * file. If the host application implements a
 ...
 * <li>This library is thread-safe but is not multi-process safe. Unless the application explicitly
...
public class LocalyticsSession {
  ...
  /**
   * Helper to obtain a new {@link android.os.HandlerThread}.
   *
   * @param name to give to the HandlerThread. Useful for debugging, as the thread name is shown in
   * DDMS.
   * @return HandlerThread whose {@link android.os.HandlerThread#start()} method has already been
   * called.
   */
  ...
  /**
   * Maps an API key to a singleton instance of the {@link com.localytics.android.LocalyticsSession.SessionHandler}.
   * Lazily initialized during construction of the
   * {@link com.localytics.android.LocalyticsSession} object.
   */
  private static final Map<String, SessionHandler> sLocalyticsSessionHandlerMap =
      new HashMap<String, SessionHandler>();
   ...
   * use
   * {@link android.content.Context#getApplicationContext()} to avoid the potential memory leak
   * incurred by maintaining references to
   ...
   * {@link #tagEvent(String, java.util.Map)}
   * , {@link #tagEvent(String, java.util.Map, java.util.List)}, or {@link #tagScreen(String)} are
   * called.
   * <p>
  ...
  public void open(final List<String> customDimensions) {
    ...
    if (null == customDimensions || customDimensions.isEmpty()) {
      ...
      mSessionHandler.sendMessage(mSessionHandler.obtainMessage(SessionHandler.MESSAGE_OPEN,
          new TreeMap<String, String>(convertDimensionsToAttributes(customDimensions))));
    }
  }
   ...
   * #open(java.util.List)}, close the session
   ...
   * @see #tagEvent(String, java.util.Map, java.util.List, long)
   */
   ...
   */
  public void tagEvent(final String event, final Map<String, String> attributes,
      final List<String> customDimensions) {
   ...
   * <ul>
   * <li>DO NOT use events, attributes, or dimensions to record personally identifiable
   * information.</li>
   * <li>The best way to use events is to create all the event strings as predefined constants and
   * only use those. This is more
   * efficient and removes the risk of collecting personal information.</li>
   * <li>Do not tag events inside loops or any other place which gets called frequently. This can
   ...
   */
  public void tagEvent(final String event, final Map<String, String> attributes,
      final List<String> customDimensions, final long customerValueIncrease) {
    if (Constants.IS_PARAMETER_CHECKING_ENABLED) {
      if (null == event) {
      ...
      if (null != attributes) {
        ...
        if (attributes.isEmpty()) {
          if (Constants.IS_LOGGABLE) {
            Log.w(Constants.LOG_TAG,
                "attributes is empty.  Did the caller make an error?"); //$NON-NLS-1$
          }
        }
        ...
        if (attributes.size() > Constants.MAX_NUM_ATTRIBUTES) {
          if (Constants.IS_LOGGABLE) {
            Log.w(Constants.LOG_TAG, String.format(
                "attributes size is %d, exceeding the maximum size of %d.  Did the caller make an error?",
                Integer.valueOf(attributes.size()),
                Integer.valueOf(Constants.MAX_NUM_ATTRIBUTES))); //$NON-NLS-1$
          }
        }
        ...
        for (final Entry<String, String> entry : attributes.entrySet()) {
          ...
          if (null == value) {
            throw new IllegalArgumentException(
                "attributes cannot contain null values"); //$NON-NLS-1$
          }
          if (0 == key.length()) {
            throw new IllegalArgumentException(
                "attributes cannot contain empty keys"); //$NON-NLS-1$
          }
          if (0 == value.length()) {
            throw new IllegalArgumentException(
                "attributes cannot contain empty values"); //$NON-NLS-1$
          }
        }
      }
    ...
    if (null == attributes && null == customDimensions) {
      mSessionHandler.sendMessage(mSessionHandler.obtainMessage(SessionHandler.MESSAGE_TAG_EVENT,
          new Triple<String, Map<String, String>, Long>(eventString, null, customerValueIncrease)));
    } else {
            /*
  ...
  public void setCustomerData(final String key, final String value) {
    ...
    mSessionHandler.sendMessage(mSessionHandler.obtainMessage(SessionHandler.MESSAGE_SET_IDENTIFIER,
  ...
  public void registerPush(final String senderId) {
    ...
    mSessionHandler.sendMessage(
        mSessionHandler.obtainMessage(SessionHandler.MESSAGE_REGISTER_PUSH, senderId));
  }
  ...
  public void handlePushReceived(final Intent intent, final List<String> customDimensions) {
    ...
    if (llString != null) {
      try {
        ...
        if (Constants.IS_LOGGABLE) {
          Log.w(Constants.LOG_TAG,
              "Failed to get campaign id or creatve id from payload"); //$NON-NLS-1$
        }
      }
    }
  }
  ...
  public void setPushRegistrationId(final String pushRegId) {
    mSessionHandler.sendMessage(
        mSessionHandler.obtainMessage(SessionHandler.MESSAGE_SET_PUSH_REGID, pushRegId));
  }
   ...
   */
  private static Map<String, String> convertDimensionsToAttributes(
      final List<String> customDimensions) {
  ...
  /**
   * Helper class to handle session-related work on the {@link com.localytics.android.LocalyticsSession#sSessionHandlerThread}.
   */
     ...
     * <p>
     * {@link android.os.Message#obj} is either null or a {@code Map<String, String>} containing
     ...
     * <p>
     * {@link android.os.Message#obj} is a {@link com.localytics.android.LocalyticsSession.Pair}
     * instance. This object cannot be null.
     */
     ...
     * <p>
     * {@link android.os.Message#obj} is a {@code Runnable} to execute when upload is complete. The
     * thread that this runnable will
     ...
     * <p>
     * {@link android.os.Message#arg1} == 1 for true (opt out). 0 means opt-in.
     */
     ...
     * <p>
     * {@link android.os.Message#obj} is a string representing the screen visited.
     */
     ...
     * <p>
     * {@link android.os.Message#obj} is a string representing the sender id.
     */
     ...
     * <p>
     * {@link android.os.Message#obj} is a string representing the push registration id.
     */
      ...
      try {
        ...
        switch (msg.what) {
          ...
          case MESSAGE_TAG_EVENT: {
            ...
            @SuppressWarnings("unchecked")
            final Triple<String, Map<String, String>, Long> triple =
                (Triple<String, Map<String, String>, Long>) msg.obj;
                ...
                if (null != getOpenSessionId(mProvider)) {
                  ...
                  if (null == attributes) {
                    ...
                    if (attributes.containsKey(AttributesDbColumns.ATTRIBUTE_CUSTOM_DIMENSION_8)) {
                      openCloseAttributes.put(AttributesDbColumns.ATTRIBUTE_CUSTOM_DIMENSION_8,
                          attributes.get(AttributesDbColumns.ATTRIBUTE_CUSTOM_DIMENSION_8));
                    }
                    if (attributes.containsKey(AttributesDbColumns.ATTRIBUTE_CUSTOM_DIMENSION_9)) {
                      openCloseAttributes.put(AttributesDbColumns.ATTRIBUTE_CUSTOM_DIMENSION_9,
                          attributes.get(AttributesDbColumns.ATTRIBUTE_CUSTOM_DIMENSION_9));
                    }
                    if (attributes.containsKey(AttributesDbColumns.ATTRIBUTE_CUSTOM_DIMENSION_10)) {
                      openCloseAttributes.put(AttributesDbColumns.ATTRIBUTE_CUSTOM_DIMENSION_10,
                          attributes.get(AttributesDbColumns.ATTRIBUTE_CUSTOM_DIMENSION_10));
                    }
                  } else {
                    openCloseAttributes = null;
                  }
          ...
          case MESSAGE_REGISTER_PUSH: {
            ...
            mProvider.runBatchTransaction(new Runnable() {
              public void run() {
                ...
                try {
                  cursor = mProvider.query(InfoDbColumns.TABLE_NAME, null, null, null,
                      null); //$NON-NLS-1$

                  if (cursor.moveToFirst()) {
                    pushRegVersion = cursor.getString(
                        cursor.getColumnIndexOrThrow(InfoDbColumns.REGISTRATION_VERSION));
                    pushRegId = cursor.getString(
                        cursor.getColumnIndexOrThrow(InfoDbColumns.REGISTRATION_ID));
                  }
                } finally {
                  if (null != cursor) {
                    cursor.close();
                ...
                if (pushRegId == null || TextUtils.isEmpty(pushRegId) || !appVersion.equals(
                    pushRegVersion)) {
     ...
     * collected. For example, calls to {@link #tagEvent(String, java.util.Map)} and {@link
     * #tagScreen(String)} will be recorded until
     * {@link #close(java.util.Map)} is called.
     * <p>
    ...
    /**
     * Projection for {@link #open(boolean, java.util.Map)}.
     */
    private static final String[] PROJECTION_OPEN_EVENT_ID = new String[] { EventsDbColumns._ID };

    /**
     * Selection for {@link #open(boolean, java.util.Map)}.
     */
    ...
    /**
     * Selection for {@link #openNewSession(java.util.Map)}.
     */
     ...
     */
...

      {
        ...
        try {
          ...
          final CursorJoiner joiner =
              new CursorJoiner(eventsCursor, PROJECTION_OPEN_EVENT_ID, blob_eventsCursor,
                  PROJECTION_OPEN_BLOB_EVENTS);
            ...
            switch (joinerResult) {
              case LEFT: {
                ...
                if (-1 != closeEventId) {
                  ...
                  if (Constants.IS_LOGGABLE) {
                    Log.w(Constants.LOG_TAG,
                        "There were multiple close events within SESSION_EXPIRATION"); //$NON-NLS-1$
                  }

                  final long newClose =
                      eventsCursor.getLong(eventsCursor.getColumnIndexOrThrow(EventsDbColumns._ID));
      ...
      if (-1 != closeEventId) {
        ...
        try {
          sessionsCursor =
              mProvider.query(SessionsDbColumns.TABLE_NAME, PROJECTION_OPEN_SESSIONS, null, null,
                  SessionsDbColumns._ID);

          if (sessionsCursor.moveToLast()) {
            if (sessionsCursor.getLong(
                sessionsCursor.getColumnIndexOrThrow(SessionsDbColumns.SESSION_START_WALL_TIME))
                >= System.currentTimeMillis() - Constants.SESSION_EXPIRATION) {
              // reconnect
              if (Constants.IS_LOGGABLE) {
                Log.v(Constants.LOG_TAG,
                    "Opening old unclosed session and reconnecting"); //$NON-NLS-1$
              }
              return;
            }
            ...
            Cursor eventsCursor = null;
            try {
              final String sessionId = Long.toString(sessionsCursor.getLong(
                  sessionsCursor.getColumnIndexOrThrow(SessionsDbColumns._ID)));
    ...
    /**
     * Opens a new session. This is a helper method to {@link #open(boolean, java.util.Map)}.
     *
    ...
    /**
     * Close a session. While this method should only be called after {@link #open(boolean,
     * java.util.Map)}, nothing bad will happen if
     * it is called and {@link #open(boolean, java.util.Map)} wasn't called. Similarly, nothing bad
     * will happen if close is called
     * multiple times.
     * <p>
     * This method must only be called after {@link #init()} is called.
     * <p>
     ...
     * java.util.Map)} is called first,
     * this method will simply do nothing if {@link #open(boolean, java.util.Map)} hasn't been
     * called.
     * <p>
  ...
  /**
   * Helper object to the {@link com.localytics.android.LocalyticsSession.SessionHandler} which
   * helps process upload requests.
   */
     ...
     * forwards the request back to {@link com.localytics.android.LocalyticsSession#mSessionHandler}
     * with {@link com.localytics.android.LocalyticsSession.SessionHandler#MESSAGE_UPLOAD}.
     * <p>
     * {@link android.os.Message#obj} is a {@code Runnable} to execute when upload is complete. The
     * thread that this runnable will
     * executed on is undefined.
     ...
     * @param sessionHandler Parent {@link com.localytics.android.LocalyticsSession.SessionHandler}
     * object to notify when uploads are completed. Cannot be null.
      ...
      try {
        ...
        switch (msg.what) {
          ...
          case MESSAGE_RETRY_UPLOAD_REQUEST: {
            ...
            mSessionHandler.sendMessage(
                mSessionHandler.obtainMessage(SessionHandler.MESSAGE_UPLOAD, msg.obj));
            break;
          }
          default: {
                        /*
     ...
     */
        /* package */
    static List<JSONObject> convertDatabaseToJson(final Context context,
      ...
      try {
        ...
        while (cursor.moveToNext()) {
          try {
            ...
            blobHeader.put(BlobHeader.KEY_DATA_TYPE, BlobHeader.VALUE_DATA_TYPE);
            blobHeader.put(BlobHeader.KEY_PERSISTENT_STORAGE_CREATION_TIME_SECONDS, creationTime);
            blobHeader.put(BlobHeader.KEY_SEQUENCE_NUMBER, cursor.getLong(idColumn));
            blobHeader.put(BlobHeader.KEY_UNIQUE_ID, cursor.getString(uuidColumn));
            blobHeader.put(BlobHeader.KEY_ATTRIBUTES, getAttributesFromSession(provider, apiKey,
                getSessionIdForBlobId(provider, cursor.getLong(idColumn))));

            final JSONObject identifiers = getIdentifiers(provider);
            if (null != identifiers) {
              blobHeader.put(BlobHeader.KEY_IDENTIFIERS, identifiers);
            }
     ...
     * @throws org.json.JSONException if a problem occurred converting the element to JSON.
     */
        /* package */
    static JSONObject getAttributesFromSession(final LocalyticsProvider provider,
      ...
      try {
        ...
        if (cursor.moveToFirst()) {
          ...
          // Sessions table
          result.put(BlobHeader.Attributes.KEY_CLIENT_APP_VERSION,
              cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.APP_VERSION)));
          result.put(BlobHeader.Attributes.KEY_DATA_CONNECTION,
              cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.NETWORK_TYPE)));
          result.put(BlobHeader.Attributes.KEY_DEVICE_ANDROID_ID_HASH, cursor.getString(
              cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_ANDROID_ID_HASH)));
          result.put(BlobHeader.Attributes.KEY_DEVICE_COUNTRY,
              cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_COUNTRY)));
          result.put(BlobHeader.Attributes.KEY_DEVICE_MANUFACTURER, cursor.getString(
              cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_MANUFACTURER)));
          result.put(BlobHeader.Attributes.KEY_DEVICE_MODEL,
              cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_MODEL)));
          result.put(BlobHeader.Attributes.KEY_DEVICE_OS_VERSION,
              cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.ANDROID_VERSION)));
          result.put(BlobHeader.Attributes.KEY_DEVICE_PLATFORM,
              BlobHeader.Attributes.VALUE_PLATFORM);
          result.put(BlobHeader.Attributes.KEY_DEVICE_SERIAL_HASH, cursor.isNull(
              cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_SERIAL_NUMBER_HASH))
              ? JSONObject.NULL : cursor.getString(
              cursor.getColumnIndexOrThrow(SessionsDbColumns.DEVICE_SERIAL_NUMBER_HASH)));
          result.put(BlobHeader.Attributes.KEY_DEVICE_SDK_LEVEL,
              cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.ANDROID_SDK)));
          result.put(BlobHeader.Attributes.KEY_LOCALYTICS_API_KEY, apiKey);
          result.put(BlobHeader.Attributes.KEY_LOCALYTICS_CLIENT_LIBRARY_VERSION, cursor.getString(
              cursor.getColumnIndexOrThrow(SessionsDbColumns.LOCALYTICS_LIBRARY_VERSION)));
          result.put(BlobHeader.Attributes.KEY_LOCALYTICS_DATA_TYPE,
              BlobHeader.Attributes.VALUE_DATA_TYPE);
          result.put(BlobHeader.Attributes.KEY_CURRENT_TELEPHONY_ID,
          ...
          result.put(BlobHeader.Attributes.KEY_CURRENT_ANDROID_ID,
          ...
          if (null != installationID) {
            result.put(BlobHeader.Attributes.KEY_LOCALYTICS_INSTALLATION_ID, installationID);
          }
          result.put(BlobHeader.Attributes.KEY_LOCALE_COUNTRY,
              cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.LOCALE_COUNTRY)));
          result.put(BlobHeader.Attributes.KEY_LOCALE_LANGUAGE,
              cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.LOCALE_LANGUAGE)));
          result.put(BlobHeader.Attributes.KEY_NETWORK_CARRIER,
              cursor.getString(cursor.getColumnIndexOrThrow(SessionsDbColumns.NETWORK_CARRIER)));
          result.put(BlobHeader.Attributes.KEY_NETWORK_COUNTRY,
    ...
    /**
     * Private helper to convert an event's attributes into a {@link org.json.JSONObject}
     * representation.
     *
     * @param provider Localytics database instance. Cannot be null.
     * @param context Application context. Cannot be null.
     * @param eventId {@link EventsDbColumns#_ID} of the event whose attributes are to be loaded.
     * @return {@link org.json.JSONObject} representing the attributes of the event. The order of
     * attributes is undefined and may
     * change from call to call of this method. If the event has no attributes, returns null.
     * @throws org.json.JSONException if an error occurs converting the attributes to JSON
     */
        /* package */
    ...
    static long getSessionIdForBlobId(final LocalyticsProvider provider, final long blobId) {
      ...
      long eventId;
      {
        Cursor cursor = null;
        try {
          cursor = provider.query(UploadBlobEventsDbColumns.TABLE_NAME,
              new String[] { UploadBlobEventsDbColumns.EVENTS_KEY_REF },
              String.format("%s = ?", UploadBlobEventsDbColumns.UPLOAD_BLOBS_KEY_REF),
              new String[] //$NON-NLS-1$
          ...
          if (cursor.moveToFirst()) {
            eventId = cursor.getLong(
                cursor.getColumnIndexOrThrow(UploadBlobEventsDbColumns.EVENTS_KEY_REF));
          } else {
                        /*
                         * This should never happen
                         */
  ...
  /**
   * Internal helper class to pass two objects to the Handler via the {@link
  ...
  /**
   * Internal helper class to pass three objects to the Handler via the {@link
   * android.os.Message#obj}.
   */
