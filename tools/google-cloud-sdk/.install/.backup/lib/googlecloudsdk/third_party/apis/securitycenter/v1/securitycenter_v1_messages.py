...
class GroupAssetsRequest(_messages.Message):
  ...
  Fields:
    compareDuration: When compare_duration is set, the GroupResult's
      ...
      was not present at the start of                compare_duration, but
      present at reference_time. * "REMOVED": indicates that the asset was
      present at the start of                compare_duration, but not present
      at reference_time. * "ACTIVE":  indicates that the asset was present at
      both the                start and the end of the time period defined by
      compare_duration and reference_time.  If compare_duration is not
      specified, then the only possible state_change is "UNUSED", which will
      be the state_change set for all assets present at read_time.  If this
      field is set then `state_change` must be a specified field in
      `group_by`.
...
class GroupFindingsRequest(_messages.Message):
  ...
  Fields:
    compareDuration: When compare_duration is set, the GroupResult's
      ...
      time. Intermediate state changes between the two times don't affect the
    ...
    groupBy: Expression that defines what assets fields to use for grouping
      ...
      * parent  The following fields are supported when compare_duration is
      set:  * state_change
    pageSize: The maximum number of results to return in a single response.
      Default is 10, minimum is 1, maximum is 1000.
...
class SecuritycenterOrganizationsAssetsListRequest(_messages.Message):
  ...
  Fields:
    compareDuration: When compare_duration is set, the ListAssetsResult's
      ...
      compare_duration is specified:  * "ADDED":   indicates that the asset
      was not present at the start of                compare_duration, but
      present at read_time. * "REMOVED": indicates that the asset was present
      at the start of                compare_duration, but not present at
