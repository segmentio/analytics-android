...
import io.segment.android.request.IRequestLayer.RequestCallback;
...
public class FlushThread extends LooperThreadWithHandler implements IFlushLayer {
	...
	public FlushThread(IPayloadDatabaseLayer databaseLayer,
			...
			public void run() {
				...
				flushNextPayload(new FlushCallback() {
					...
					public void onFlushCompleted(boolean success, Batch batch) {
						...
						if (success) Logger.d("Flush op success. [" + batch.getBatch().size() +  " items]");
						else Logger.w("Flush op failed. [" + batch.getBatch().size() +  " items]");
